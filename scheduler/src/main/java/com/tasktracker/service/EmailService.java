...
public class EmailService {
    ...
    public EmailMessage createEmailMessage(User user){
            ...
            List<Task> incompleteTasks = tasks.stream().filter(task ->
                    Boolean.FALSE.equals(task.getCompleted())).toList();
                List<Task> todayCompleted = tasks.stream()
                        .filter(task -> task.getCompletedAt() != null &&
                                !task.getCompletedAt().isBefore(startOfDay) &&
                                task.getCompletedAt().isBefore(endOfDay)).toList();
            List<String> incompleteTitles = incompleteTasks.stream().map(Task::getTitle).limit(5).toList();
            List<String> titles = todayCompleted.stream().map(Task::getTitle).limit(5).toList();
